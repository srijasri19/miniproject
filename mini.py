...
class Application(tk.Frame): 
    ...
    def build_interface(self):
        ...
        self.clock = tk.Label(self, text="00:00:00", font=("algerian", 20), width=10)
        self.clock.grid(row=1, column=1, stick="S")

        self.time_label = tk.Label(self, text="hour   min   sec", font=("algerian", 10), width=15)
        ...
        self.pause_button.grid(row = 3,column=2, sticky = "NE")

        self.master.bind("<Return>", lambda x: self.start())
        self.time_entry.bind("<Key>", lambda v: self.update())

        self.pause_button.configure(bg="sky blue")
        self.quit_button.configure(bg="crimson")

        self.power_button.configure(bg="green")
        self.reset_button.configure(bg="gold")

        self.time_label.configure(bg="light pink")
        self.time_entry.configure(bg="light yellow")
